...
export function LastMessageDecorations({
  ...
  const { isActiveInterrupt, handleApprove, handleReject, handleSubmit } =
  ...
  const pendingDisambiguation =
    getPendingDisambiguation(saisUiData.raw) ??
    (msgPendingDisambiguation
      ? getPendingDisambiguation({
  ...
  const handleClarificationSelect = useCallback(
    (value: string) => {
      ...
      threadSubmit(
        { messages: [...threadMessages, clarifyMsg] } as Record<
          string,
          unknown
        > as any,
        {
  ...
  const handleDisambiguationSelect = useCallback(
    (entityName: string, action: string, nodeId?: string) => {
      const text =
        action === "skip" ? "None of these match what I meant" : action;
      ...
      threadSubmit(
        { messages: [...threadMessages, disambigMsg] } as Record<
          string,
          unknown
        > as any,
        {
    ...
    <>
      ...
      {hasBlocks ? (
        ...
        <>
          ...
          {/* AI text content */}
          {contentString.length > 0 &&
            !getInterruptDecision(msgResponseMeta) &&
            !(
              pendingDisambiguation &&
              pendingDisambiguation.candidates.length > 0
            ) && (
          ...
          {remediationProposals && remediationProposals.length > 0 && (
            <div
              className="mt-3"
              data-testid="remediation-proposals"
            >
              <BatchReview
                batchId={
                ...
                <BlockerMessage
                  ...
                  onAction={(action?: string) => {
                    ...
                    if (text) {
                      ...
                      thread.submit(
                        { messages: [...thread.messages, actionMsg] } as Record<
                          string,
                          unknown
                        > as any,
                        {
